<?php
	...
	$db = null;
			...
			if(!empty($tagmap)){
                ...
                echo "<p><strong>Tag Map</strong> (These Booru Tags are mapped to " . $tag[0] . ")</p><p>-</p>";
	...
	<script type="text/javascript">
		...
		function TagSuggestions(data, id)
		{
			...
			$.ajax({
				url: 'TagSuggestionsAjax.php?txt=' + encodeURIComponent(data),
				type: 'get',
				dataType: 'JSON',
		...
		{
			$.ajax({
				url: 'CategoryAjax.php?txt=' + encodeURIComponent(data),
				type: 'get',
				dataType: 'JSON',
				success: function(response){
		...
		function SubmitParent()
		{
			...
			$.ajax({
				url: 'AddParentAjax.php?tagid=' + tagid + '&parent=' + encodeURIComponent(parent_tag) + '&cat=' + cat,
				type: 'get',
				dataType: 'JSON',
				success: function(response){
					if(response.length > 0)
		...
		function SubmitChild()
		{
			...
			$.ajax({
				url: 'AddChildAjax.php?tagid=' + tagid + '&child=' + encodeURIComponent(child_tag),
				type: 'get',
				dataType: 'JSON',
				success: function(response){
					if(response.length > 0)
		...
		{
			$.ajax({
				url: 'RemoveTagMapAjax.php?boorutag=' + encodeURIComponent(boorutag) + '&tagid=' + tagid + '&bs=' + bs + '&namespace=' + encodeURIComponent(namespace),
				type: 'get',
				dataType: 'JSON',
				success: function(response){
					if(response.length > 0)
