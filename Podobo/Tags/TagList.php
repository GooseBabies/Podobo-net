...
?>
		...
		<?php
			...
			echo "<p class='w3-center'><b>Tags: " . $tagcount . "</b></p>";
			...
			//add alias, order, desc (descending), only show up if not default
			
			if($page != 1) 
			{
				echo "<a href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") . "'><i class='fas fa-angles-left fa-2x'></i></a>";
				echo "<a href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") ."&page=" . ($page - 1) . "'><i class='fas fa-angle-left fa-2x'></i></a>";	
			};
			
			for($k = $offset; $k <= $end_page; $k++)
			{
				if($k == $page){
					echo "<a class='current-page' href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") . "&page=" . $k  . "'>";
					echo $k;
					echo "</a>";
				}
				else{
					echo "<a class='other-page' href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") . "&page=" . $k  . "'>";
					echo $k;
					echo "</a>";
				}
			}
			...
			if($page != $pagecount) 
			{
				echo "<a href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") ."&page=" . ($page + 1) . "'><i class='fas fa-angle-right fa-2x'></i></a>";
				echo "<a href='TagList.php?search=" . ($search==""?"":htmlspecialchars($search)) . ($cat==-1?"":"&cat=" . $cat) . ($aliases==1?"&aliases=1":"") . ($order==0?"":"&order=" . $order) . ($desc==0?"":"&desc=1") ."&page=" . $pagecount . "'><i class='fas fa-angles-right fa-2x'></i></a>";
			};
			echo "</div>";
