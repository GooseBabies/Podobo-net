<?php
    ...
    try{
        ...
        if($newtagid != -1 && $tagid != $newtagid){
            ...
            $sql = $db->prepare("Select media_id from media_tags where tag_id = :tag_id;");
            $sql->bindValue(':tag_id', $tagid, SQLITE3_TEXT);
            $result = $sql->execute();
            ...
            for($i = 0; $i <= count($ids) - 1; $i++){
                ...
                $sql = $db->prepare("Update media_tags set tag_id = :newtagid where tag_id = :oldtagid");
                $sql->bindValue(':newtagid', $newtagid, SQLITE3_INTEGER);
                $sql->bindValue(':oldtagid', $tagid, SQLITE3_INTEGER);
                $result = $sql->execute();
                ...
                $sql->bindValue(':tagid', $newtagid, SQLITE3_INTEGER);
                $sql->execute();
