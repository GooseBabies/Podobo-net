<?php
    ...
    //if check whitelist flag is marked echo 2

    //new return array pos 0: state (1 for next media, 2 for decide complete, 3 for whitelist checker) and pos 1: id (media id for state 1, "" for state 2, whitelist id for state 3)

    if(!$mediaid){
        $sql = $db->prepare("update processed set media=1");
        $sql->execute();        
    }
    ...
    elseif($mediaid){
        echo json_encode(array(1,$mediaid[0]));
    }
    else{
