<?php
	...
	if(isset($_GET["dupe"])) { $dupe = $_GET["dupe"]; } else { $dupe = ""; }; 
    ...
    for($q = 0; $q <= count($items) - 1; $q++)
    {
        if($items[$q] != ""){
            ...
            if($itemss[1] == 0){
                ...
                $sql->bindValue(":kept", 1, SQLITE3_INTEGER);
                $sql->bindValue(":id", $itemss[0], SQLITE3_INTEGER);
                $sql->execute();
            }
            else{
                //trash
                $sql = $db->prepare("update media set kept=:kept where id=:id");
                $sql->bindValue(":kept", 0, SQLITE3_INTEGER);
                $sql->bindValue(":id", $itemss[0], SQLITE3_INTEGER);
                $sql->execute();                
            }            
    ...
    //mark dupe row processed
    $sql = $db->prepare("update dupes set processed=:processed where id=:id");
    $sql->bindValue(":processed", 1, SQLITE3_INTEGER);
